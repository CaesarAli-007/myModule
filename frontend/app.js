...
function setAuthState(authed)
{
    if(authed)
    {
        hide(showRegisterBtn); hide(showLoginBtn); show(logoutBtn);
        hide(authForms); show(domainSelection);
    } 
    else 
    {
        show(showRegisterBtn); show(showLoginBtn); hide(logoutBtn);
        hide(domainSelection); hide(questionnaire); hide(success);
        show(authForms); show(registerForm); hide(loginForm); hide(resetRequestForm); hide(resetConfirmForm);
    }
}
...
// UI events for showing forms
showRegisterBtn.onclick = ()=>{ show(registerForm); hide(loginForm); hide(resetRequestForm); hide(resetConfirmForm) }
showLoginBtn.onclick = ()=>{ show(loginForm); hide(registerForm); hide(resetRequestForm); hide(resetConfirmForm) }
logoutBtn.onclick = ()=>{ localStorage.removeItem('token'); token=null; setAuthState(false) }
...
registerForm.addEventListener('submit', async (e)=>{
    ...
    {
        console.error(err); alert('Network or server error during registration')
    }
})
...
loginForm.addEventListener('submit', async (e)=>{
    ...
    try
    {
        ...
        if(res.ok && j.token)
        {
            token = j.token; localStorage.setItem('token', token); setAuthState(true)
            // Optionally fetch profile or show domain selection
        } 
        else 
        {
    ...
    {
        console.error(err); alert('Network or server error during login')
    }
})
...
if(forgotLink)
{
    forgotLink.addEventListener('click', (e)=>{
        e.preventDefault()
        hide(loginForm); show(resetRequestForm)
    })
}
...
resetRequestForm.addEventListener('submit', async (e)=>{
    ...
    try
    {
        ...
        const j = await res.json()
        alert(j.message || 'If account exists, reset link printed to server logs (dev).')
        // go back to login
        hide(resetRequestForm); show(loginForm)
    }
    catch(err)
    {
        console.error(err); alert('Network or server error requesting reset')
    }
})
...
resetConfirmForm.addEventListener('submit', async (e)=>{
    ...
    const payload = { token: form.get('token'), password: form.get('password') }
    try
    {
        ...
        const j = await res.json()
        alert(j.message || 'Password reset result received')
        hide(resetConfirmForm); show(loginForm)
    }
    catch(err)
    {
        console.error(err); alert('Network or server error performing reset')
    }
})
...
document.querySelectorAll('.domain-btn').forEach(btn=>{
    btn.addEventListener('click', ()=>{
        ...
        qTitle.textContent = currentDomain + ' — Initial Screening'
        renderQuestions(currentDomain)
        hide(domainSelection); show(questionnaire)
    })
})
...
function renderQuestions(domain){
    ...
    bank.forEach((q,i)=>{
        ...
        const label = document.createElement('label'); label.textContent = (i+1) + '. ' + q
        const textarea = document.createElement('textarea'); textarea.name = 'q'+i; textarea.rows=2; textarea.placeholder='Type your answer here...'
        div.appendChild(label); div.appendChild(textarea); questionsDiv.appendChild(div)
    })
}
...
qaForm.addEventListener('submit', async (e)=>{
    ...
    if(!currentDomain){ alert('No domain selected'); return }
    ...
    for(let i=0;i<bank.length;i++)
    {
        ...
        answers.push({ q: bank[i], a: (field && field.value) ? field.value.trim() : '' })
    }
    const payload = {
    ...
    try
    {
        ...
        if(res.ok)
        {
            hide(questionnaire); show(success);
            summary.textContent = `Saved for domain ${currentDomain}. Document id: ${j.id || 'n/a'}`
        }
        else
        {
            if(j && j.message) alert(j.message); else alert('Failed to save responses')
        }
    }
    catch(err)
    {
        console.error(err); alert('Network or server error saving responses')
    }
})

// Optionally allow user to go back to domain selection (not required but useful)
// Add a simple event delegation — if you want a back button, just add element and hook it here.
// Example (uncomment and use if you add a button with id 'back-to-domains'):
// const backBtn = el('back-to-domains')
// if(backBtn) backBtn.addEventListener('click', ()=>{ hide(questionnaire); show(domainSelection); })

// Check URL for reset token (support ?reset=TOKEN)
(function checkUrlForToken(){
    try
    {
        ...
        if(params.get('reset'))
        {
            hide(authForms); show(resetConfirmForm)
            // fill token field if exists
            if(resetConfirmForm.token) resetConfirmForm.token.value = params.get('reset')
        }
    }
    catch(e){ /* ignore */ }
})()

// On load, optionally verify token with server (light check)
// If you want to validate token on page load uncomment this and implement /api/me endpoint
/*
(async function verifyToken(){
    ...
    try
    {
        ...
        if(res.ok)
        {
            // user is valid — leave in authenticated state
            setAuthState(true)
        }
        else
        {
            // invalid token
            localStorage.removeItem('token'); token = null; setAuthState(false)
        }
    }
    catch(err)
    {
